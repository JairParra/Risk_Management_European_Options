...
```{r}
#We need to filter out duplicate moneyness and maturity pairs
Moneyness <- call_put_data$m[1:10]
Tau <- call_put_data$tau[1:10]

table

IV <- matrix(NA, nrow = length(Moneyness), ncol = length(Tau))

#Creating a surface 
for(i in 1:length(Moneyness)){
  for(j in 1:length(Tau)){
    IV[i,j] <- f_sig(alpha, Moneyness[i], Tau[j])
  }
}
#Try plotting now
