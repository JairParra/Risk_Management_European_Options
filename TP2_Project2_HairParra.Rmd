...
\begin{aligned}
\log(R_{t+1})&= \log(P_{t+1} - P_{t}) - \log(P_{t}) \\ 
\implies \log(P_{t+1} - P_{t})  &= \log(R_{t+1}) + \log(P_{t}) \\ 
\implies P_{t+1} - P_{t} &= \exp\left(  \log(R_{t+1}) + \log(P_{t})  \right) \\ 
\implies P_{t+1} &= \exp\left(  \log(R_{t+1}) + \log(P_{t})  \right) + P_t  \\ 
\implies P_{t} & = \exp\left(  \log(R_{t}) + \log(P_{t-1})  \right) + P_{t-1} 
\end{aligned}
$$
...
log_Pt <- log(Pt) # llog of current prices log(P_{t})
