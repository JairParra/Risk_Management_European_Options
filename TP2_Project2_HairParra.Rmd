...
sp500_sim_rets <- t(apply(sim_inv_sp500, 1, function(x){f_forecast_y(theta=theta, 
                                                                     sig2_prev = sig2_prev, 
                                                                     y_prev = y_prev, 
                                                                     resids_next = x)$y_next}))
colnames(sp500_sim_rets) <- c("T+1", "T+2", "T+3", "T+4", "T+5")
head(sp500_sim_rets)
```
...
```{r}
# apply to all rows and pack into a matrix 
vix_sim_rets <- t(apply(sim_inv_vix, 1, function(x){f_forecast_x(phi=phi,
                                                                 x_prev = x_prev, 
                                                                 resids_next = x)}))
colnames(vix_sim_rets) <- c("T+1", "T+2", "T+3", "T+4", "T+5")
head(vix_sim_rets)
```
...
abline(v=ex_vix_forecast, col="blue")
legend(x="topleft", 
