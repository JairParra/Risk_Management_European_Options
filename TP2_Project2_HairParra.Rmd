...
## test: specific price 
price_option(T=20, K=1600, calls, rf_mat, stock = NA, S_t = S_t, IV = IV)
```
...
```{r}
# First Call Option 
price_option(T=20, K=1600, calls, rf_mat, S_t = S_t, IV = IV)
```

```{r}
# Second Call Option 
price_option(T=40, K=1605, calls, rf_mat, S_t = S_t, IV = IV)
```

```{r}
# Third Call Option 
price_option(T=40, K=1800, calls, rf_mat, S_t = S_t, IV = IV)
```
...
B = 10 # set number of bootstraps
n_ahead = 5 # days aheade to produce samples 

# vector version since ignoring dates and using full past data
sp500_vec <- as.vector(sp500_rets) 
vix_vec <- as.vector(vix_rets)

# preallocate matrices to store simulations 
sim_sp500 <- matrix(NA, nrow = B, ncol=5)
sim_vix <- matrix(NA, nrow = B, ncol=5)

# assign days ahead 
colnames(sim_sp500) <- c("T+1", "T+2", "T+3", "T+4", "T+5")
colnames(sim_vix) <- c("T+1", "T+2", "T+3", "T+4", "T+5")

# perform bootstrapping 
for(b in 1:B){
  
  # Obtain the bootstrapped samples from the data 
  ...
  R <- matrix(data = c(sig[1], r, r, sig[2]), # correlation matrix
              nrow = 2,
              ncol = 2,
              byrow = TRUE)
...
sim_price_sp500 <- f_return_to_price(spT, sim_sp500)
sim_vol_vix <- f_return_to_price(vixT, sim_vix)
...
PL_mat <- matrix(NA, nrow(sim_price_sp500), ncol(sim_price_sp500))

# wrapper for price_option 
prc_opt <- function(T, K, calls, rf_mat, price_vec, IV){ 
  # Calculate the options for all input S_t 
  sapply(price_vec, function(y){price_option(T=T, # maturity
                                             K=K, # strike 
                                             calls, # calls matrix 
                                             rf_mat, # matrix of risk free structure
                                             stock = NA, # ignore 
                                             S_t = y, # specific price
                                             IV = IV)$Call}) # implied volatility + extract Call price
}
...
# looop through simulated prices 
for(t in 1:ncol(sim_price_sp500)){
  ...
  # c1_vec <- prc_opt(T1-t, K, calls, rf_mat, prices_t) 
}
```
