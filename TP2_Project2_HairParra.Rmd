...
```{r, echo=FALSE}
### this block not included in the PDF
...
```{r, echo=FALSE}
## Saving this data into an external datafile (not included in out pdf) 

# pack the reformatted data 
Market_clean <- list(sp500 = sp500, vix = vix, rf = rf_mat, calls=calls, puts=puts)

# save to external folder 
save(Market_clean, file = here("data_out", "Market_clean.rda"))
```
...
$$
f(x) = \dfrac{1}{(2\pi)^{d/2}|\Sigma|^{-1}} \exp\left( -\dfrac{1}{2}(x - \mu)^{T}\Sigma^{-1}(x - \mu) \right), 
$$

where the mean vector $\mu$ and covariance matrix $\Sigma$ are given by: 

$$
\mu := \mathbb{E}[X] \;\;\;\;,\;\;\; \Sigma := Cov(X) = \mathbb{E}[(X-\mu)(X-\mu)^{T}] , 
$$
...
```{r fig.height=9, fig.width=11, fig.align='center', message=FALSE}
# flatten the matrices 5-days ahead simulated P/L for the three options 
...
plot(U, pch = 20, cex = 0.9, main= "Two Student-t Marginals Scatterplot", xlab="SP500 Rets Fit", ylab="VIX Rets Mean")
```
...
where: 
- $m=K/S$ is the **monyness**.
- $\tau$ is the time to maturity of the option in years. 
- $\alpha_1, \dots, \alpha_4$ are model parameters.
...
We are assuming the same parametric model but shifted by the VIX difference. There is a *distance* between the model and the data, we need to keep the same distance and project it forward when repricing. 
