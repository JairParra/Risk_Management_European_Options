...
B = 10000 # set number of bootstraps
...
First, we code a function that will compute the option price 
...
opt_price_mats <- lapply(rep(1, 3), # generate three empty matrices of compatible sizes
                  function(x){
                    matrix(NA, 
                           nrow(sim_price_sp500), 
                           ncol(sim_price_sp500),
                           dimnames=list(seq(1:nrow(sim_price_sp500)),
...
names(opt_price_mats) <- c("PL1", "PL2", "PL3")
...
$$
\begin{aligned}
CF_T^{\text{Long Call}} = \max(S_T - K, 0) \\ 
CF_T^{\text{Long Put}} = \max(K - S_T, 0)
\end{aligned}
$$
